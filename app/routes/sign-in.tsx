import { MetaFunction, LoaderFunction, ActionFunction, json, useFetcher, useLoaderData, redirect } from "remix"
...
5. the session is empty so it redirects back to the home pag 
...
const loader: LoaderFunction = async ({ request }) => {
  ...
  if (id && sessionToken && sessionId) {
    ...
    if (!user) return json({ errors: { server: "There was an issue signing in." } })
    ...
    <div>
      <div className="bg-white dark:bg-gray-900 py-8 px-4 shadow sm:rounded-lg sm:px-10">
        {form?.data?.success?.emailSent ? (
          <p>Fake email sent. Go to localhost:3000/sign-in?token=93742 to test the magic link</p>
        ) : (
          <form.Form method="post">
            <div className="space-y-6">
              <label>Email</label>
              <input name="email" placeholder="type anything" />
              {data?.errors?.server ? <p className="text-red-600 dark:text-red-700 text-sm">{data.errors.server}</p> : null}
              <div>
                <button
                  type="submit"
